...
以长期股东视角维护上市公司研究。每次研究以商业模式与核心驱动为主线，产出结构化数据与静态分析页面：双源交叉验证的采集数据、六个维度的分析（每个维度带追问与回答）、七个维度的结论与信心度评分（六个分析维度 + 企业文化，满分 10，价值投资视角下的看多程度；企业文化为客观陈述、只转述已落盘证据）、四类策略建议（空仓者 / 持仓者 / 卖出信号 / 加仓信号，逐条绑定可观察触发条件）。
...
- 研究方法的评估记录在 `research/evals/`，三份**追加式、不追溯改写**的 JSONL：`events.jsonl`（校验与合并工具运行时自己写）、`runs.jsonl`（一次研究一行的评估记录）、`defects.jsonl`（每次评分的「最差的一处」）。它是可纵向比较的长期记录，**纳入 git，不属于 `tmp/`**；只追加、不回改，改写历史评价会让纵向比较失效。
...
- 第 7 步评估与反馈是研究方法自身的观察机制：机器指标由工具运行时自己记账，阅读评分默认由第 7 步启动的评分 Agent 读完成品页当场打（操作者只答自己才知道的行为两问），唯一正文是 `docs/research/workflow/07-evaluation-and-feedback.md`。**改研究方法必须能指出这次改动修的是 `research/evals/defects.jsonl` 里哪一条已记录的失败**，没有对应缺陷记录的改动是凭感觉，不该进。
...
- 公司目录由第一次 Write 落盘时自动创建，**不要 `mkdir` 预创建**：空目录进不了 git，只会在流程中途触发路径校验噪音。
