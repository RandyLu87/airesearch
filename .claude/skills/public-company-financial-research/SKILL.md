...
**执行 `docs/research/public-company-financial-research.md`，按 1–7 步完成：数据采集 → 多维度分析 → 分析总结 → 数据校验 → 渲染网站 → 更新首页 → 评估与反馈。** 每一步的唯一正文在 `docs/research/workflow/<NN>-*.md`。
...
# 第 7 步：评估与反馈（读完报告当场打分；不做这一步，本次研究不算完成）
npm run research:feedback -- --company <id>
```
