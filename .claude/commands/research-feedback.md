...
7. **这份报告最差的一处是什么**（一句话即可，但要具体到能照着改；**暂时没发现就直说，这是合法答案**，不必硬找一条）
...
告诉用户第 7 项最重要：五个分数会随习惯往上漂直到失去区分度，「最差的一处」不会，而且它会直接进缺陷日志，成为下次改研究方法的依据。同时说明**暂时没发现是合法答案**——不要为了填满而编一条琐碎的抱怨，那只会把缺陷日志变成噪音。
...
`worstPart` 用用户原话，不要替他润色。**用户说暂时没发现时，把 `worstPart` 写成空字符串**（脚本会记成 `noneFound: true` 并跳过缺陷记录）——不要替他编一条，也不要追问逼他找一条出来。

`defectStep` 由你从「最差的一处」判断归属哪一步，判断不了就填 `unspecified`；没有缺陷时该字段可省略。`correctionMessages` 用户没答就省略该字段（不要填 0——那是「一次都没纠正」的意思）。
