...
- **讲稿加工** — 画面只放重点、语音负责展开：画面文本一律收成前导句（确定性），
  解说可选走 LLM 口语稿并由 `scripts/brief.py` 的数字白名单 + 时长闸门放行
...
**默认纯拼接，不调用 LLM**：解说词只转述原文字段，分数直接取 `confidence` 原值，模板只补
「第几、维度名、信心度 X 分」这类连接词——原文没有的判断和数字，成片里也不会有。
带 `--brief` 时解说改用讲稿加工件的口语稿（唯一一处 LLM 改写，见「讲稿加工」一节），
但那份稿子必须先过 `scripts/brief.py` 的数字白名单与时长闸门。
...
| `moat-overview` | 五类壁垒并排五张卡：类型 + 判定标签（大号、按判定着色）+ 判定说明（小号压暗、封三行）|
...
## 讲稿加工（`scripts/brief.py`）——唯一一处 LLM 改写

前面几步是纯拼接：解说词逐字取自研究 JSON。好处是绝不编造，代价是**读起来像报告不像
人说话**——研究正文一句话动辄一两百字，从句套从句，念出来听不懂。

所以出片前可以加一道加工：把研究结论改写成**口语解说**（`spoken`）与**画面重点**
（`headline`）。改写由 LLM 做（这是判断，不是字符串处理），**放行由这个脚本做**。

```bash
# 1) 先出一份控时基准稿（不带 --brief），它给出每条分镜应得的秒数
npm run script -- --summary … --analysis … --collection … --out out/script/<id>.base.json

# 2) 由 LLM 产出 out/brief/<id>.json，然后校验；不过就不许出片
python3 scripts/brief.py check --brief out/brief/<id>.json \
  --summary … --analysis … --collection … --storyboard out/script/<id>.base.json

# 3) 带上讲稿重新生成分镜稿
npm run script -- --summary … --brief out/brief/<id>.json --out out/script/<id>.json
```

`pipeline.mjs` 会自动发现 `out/brief/<id>.json` 并按上面三步跑；没有加工件就退回纯拼接
并在 stderr 提示一行。`run.json` 里的 `narration` 记 `brief` 还是 `verbatim`——
**两者的人工核对方式不一样**：逐字转述只要看通不通顺，口语稿必须回头核对判断有没有走样。

### 加工件长什么样

```json
{"companyId": "hk-3690-meituan",
 "scenes": {"dimension-valuation": {"headline": "估值6分：中性价只高5.1%", "spoken": "第六个维度，估值…"}}}
```
...
| 闸门 | 规则 | 为什么 |
| --- | --- | --- |
| **数字白名单** | 讲稿里每个数字都必须能在研究 JSON 里原样找到 | 最重要的一条。把 `56.68亿` 说成 `57亿` 也算越界——四舍五入是改口径，不是改文风 |
| **时长** | 逐条 ≤ 该分镜控时预算（原估时 × 1.25，地板 26s）；整片 ≤ 目标上限 | 口语化几乎一定比原文长。美团第一版讲稿数字全对，却把片长从 292s 撑到 634s |
| **引用** | 不许出现 URL、字段路径 | 与 `strip_speech_noise` 同一条纪律 |
| **形状** | `headline` ≤ 24 字；分镜 id 必须是生成器认得的 | 画面重点超了就又变成大段文字；不许凭空加分镜 |

**估时一律复用 `script_gen.estimate_seconds`，不在这里另写一套**：它按「数字逐位念」估算，
而讲稿是数字密集的，用 `字数 ÷ 语速` 会系统性低估——实测同一份稿子两种算法差 6 秒，
表现为「校验放行、下一步的控时闸门却拦下来」，比没有闸门更难查。

校验器只认「有没有越界」，不判断「改写得好不好」——后者是人读了才知道的事。

### 两处代价，写在明面上

1. **`beats` 逐条点亮关闭**。它靠「这条要点由 narration 的第几句念出来」定位，解说整段
   换掉之后那个下标就失效了。宁可整屏一起亮，也不要让要点亮在错的句子上。
2. **时长要重算**。控时阶梯按原稿跑过，换稿后长度会变；重算后掉出区间就如实报
   `withinTarget: false` 并以退出码 1 中断，不假装达标。

### 长文本：切，但不改写

研究正文里的结论天然是长句，直接搬上屏会**从两个方向压垮版面**。两处都只按标点切，
一个字都不重写——切的是版面，不是判断：

| 位置 | 处理 | 不做会怎样 |
| --- | --- | --- |
| **制作过程的自述** | 一律不上屏：删掉了「原报告共 N 条，本片按控时只播报前 M 条」这类说明，空态措辞也去掉「原报告」字样；研究流程用语（「第 2 步」「已落盘」）由 `strip_process_talk` 摘掉 | 观众不知道这支片子怎么排的时长，也不知道研究有几步。屏幕上出现这些词只会让人以为漏看了什么。**这层账没有丢**，只是搬回它该在的地方：分镜稿的 `itemsAvailable` / `spokenCount` 与 `omissions` |
| **画面全部文本** | `apply_screen_leads` 在分镜定稿后统一收成前导句：一句一屏的位置（定位/结论/建议）58 字，并排要点 42 字，超出补省略号 | 美团实测有 46 处画面文本超 40 字、最长一条 1091 字。整段搬上屏观众十几秒读不完，于是既没读也没听，重点还被埋掉 |
| **判定** `moat.types[].verdict` | `split_verdict` 按第一个标点切成「标签 + 说明」：标签进大号字与解说，说明留在卡片里小号压暗、封三行 | 契约写的是「存在｜不存在｜待验证」，研究侧常写成整段。整段用 42px 铺进五列之一会撑出卡片、盖住字幕条 |
| **字幕** | `chunkSentence` 把超过 70 字的一句按标点切成几条，各自在**本句的时间区间内**按字数摊 | 字幕框只留 3 行高度，一句两百多字要占六行；它从底部往上长，会盖住正文 |

两处都守同一条：**切点必须是原文里的标点**，拼回去要与原文逐字相同（回归用例直接断言这一条）。
上限之内找不到任何标点时字幕才硬切——切错一个词是瑕疵，盖住正文不是。

删掉过程自述时仍守住一条：**不能把裁剪讲成缺失**。触发条件被控时裁光时画面什么都不写
（不作断言），只有原报告本来就没有这一项时才写「暂无触发条件」。
字幕的 70 字上限是从 `Scenes.tsx` 的字号、行高、行数上限推出来的，**改那边的版面常量要回来改这个数**。

### 图表取数：先归一，再画

采集文件里同一个字段的写法五花八门，`scripts/visuals.py` 在画之前统一做两件事。两件都是
**认不出来就整张图消失、且不报错**的那类问题，所以各自带回归用例（`scripts/test_visuals.py`）：

| 归一 | 要处理的实际写法 | 不做会怎样 |
| --- | --- | --- |
| **量级与币种** | `百万元` / `人民币百万元` / `RMB million` / `RMB百万` / `百万USD` / `HK$ million` / `千美元`；以及量级被误写在 `currency` 位上的 `{"unit": "currency", "currency": "RMB百万"}` | `read_amount` 返回 `None`，趋势图退回文字卡。曾经 16 家里有 7 家因此没有营收趋势图 |
| **年度序列排序** | 采集文件新→旧、旧→新两种排法都有 | 下游全按升序假定：折线图把逐年增长画成一路下滑，KPI 拿五年前那一年当「最新」。**画得出来但画反了，比没画更危险** |

两处都守同一条底线：**认不全就不猜**。年份解析不出或有重复时保持原序不动；量级认不出时
原样保留并记进 `omissions`。理想汽车的 `netProfit` 拆成 GAAP / Non-GAAP 两个口径，
这里一律不画——替读者挑一个会计口径不是画图该干的事。
...
- **朗读里的英文残留只解决了一半**：字段路径与 URL 已经被 `strip_speech_noise` 挡住（16 家
  全部干净），常见财经缩写已进词表，中英对照名在解说里也摘掉了——**括注版**
  （`哔哩哔哩 (Bilibili Inc.)`）与**不带括号版**（`美团 Meituan`、
  `核心本地商业Core Local Commerce`）都认，画面上照旧显示全称。没有空格的**单个**拉丁词
  不动（`增值服务VAS`、`小米SU7`）——那是名字的一部分，摘掉就把名字念错了，比念出英文更糟。
  但产品与机构专名
