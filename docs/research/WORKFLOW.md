...
**治理的纵向证据来自管理层承诺与逐笔资本配置记录。** 读 `docs/research/management-playbook.md`：管理层可判定的承诺逐条记录、逐条结算，并购/回购/分红/新业务投入逐笔留下当时的估值坐标与战略理由。承诺记录追加式、不追溯改写；覆盖最近三年，没有可录条目时明确写出覆盖起始时点——「没有承诺」和「没查」必须能区分。
