...
指标定下来之后声明护城河：只写 1–3 条**真正起作用的**，不逐类打勾。每条写明类型、它作用在因果链哪一环、趋势（变宽/稳定/变窄/待验证），以及 breaker——什么能摧毁它、在被摧毁前能观察到什么前置信号。**每条护城河必须至少引用一个已声明的驱动指标 ID**；找不到指标支撑的护城河先怀疑它不存在。用参照类推导 breaker 的方法见 playbook 第 5 节。

> `businessModel.moat` 字段与引用完整性校验尚未落地（ADR-0018）。在 schema 落地前，按上述结构写进 `sections` 的商业模式一节，含驱动指标 ID，以便日后原样迁入字段。
...
反向估值算完后写**分歧点**：市场在哪一个已声明的驱动上与自己的基准假设不同、双方各假设什么路径、如果市场对了自己错在因果链哪一环。分歧必须锚在一个 `driverMetrics[].id` 上——「竞争加剧」「监管风险」对任何公司在任何价格都成立，因此解释不了任何具体的价差。隐含预期与自己的假设几乎重合时如实写重合，并说明当前价格不提供安全边际。

> 结构化的分歧点字段尚未落地。在落地前写进现有的 `valuation.currentExpectation`，四件事逐一写清，不要压缩成一句概述。
