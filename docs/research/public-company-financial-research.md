...
执行 `docs/research/workflow/03-analysis-summary.md`（本步骤唯一正文）。**使用 Task 工具启动一个独立评估 Agent**，只基于第 1、2 步已落盘的数据与分析做评估、不重新采集：对六个维度——生意质量、护城河、管理层、最大风险、文明趋势、估值——逐一给出结论总结与信心度评分（总分 10 分，价值投资视角下的看多程度，越高越看多；证据不足向中性收敛）；对四类主要策略——空仓者、持仓者、卖出信号、加仓信号——给出绑定可观察触发条件的专业建议。汇总按 `docs/model/financials—summary-template.json` 落盘到 `research/companies/<company-id>/`。
