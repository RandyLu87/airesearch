...
以下三条与研究内容无关，只关乎流程能不能跑完。违反任何一条都会让流程在中途失败，且失败现象离原因很远。

1. **不预创建公司目录**。`research/companies/<company-id>/` 由第一次 Write 落盘时自动创建，**不要先 `mkdir`**。空目录不能被 git 提交，却会让 Stop hook 的路径校验报「既没有新流程产出也没有旧流程存档」，把正在跑的研究打断。（校验器已跳过空目录，此条仍是规范：预创建没有任何收益。）

2. **启动 Agent 的回合不能结束**。Task / Agent 工具是异步的，「启动」与「拿到结果」是两件事。启动后必须在**同一回合内等到它返回**再往下走，不要用「已在后台采集，完成后我再继续」这类话结束回合——回合一结束，Stop hook 就会对着还没落盘的半成品做校验；更要命的是用户此刻的任何输入都会杀掉后台 Agent，未落盘的工作全部丢失，只能从零重来。

3. **先落盘，再交给 Agent**。凡是主会话用确定性命令几秒就能取到的结构化数据（`cnstock_data.py` / `twstock_data.py` / FMP MCP），一律主会话自己取、**立刻落盘**，不要打包进 Agent 的任务里；Agent 只承担需要检索与判断的部分。这样任一环节被打断，磁盘上都留着可续跑的中间产物。
...
执行 `docs/research/workflow/03-analysis-summary.md`（本步骤唯一正文）。**使用 Task 工具启动一个独立评估 Agent**，只基于第 1、2 步已落盘的数据与分析做评估、不重新采集：对七个维度——生意质量、护城河、管理层、最大风险、文明趋势、估值、企业文化——逐一给出结论总结与信心度评分（总分 10 分，价值投资视角下的看多程度，越高越看多；证据不足向中性收敛；企业文化维度为客观陈述，只转述第 2 步管理层维度已落盘的可观察证据，不编造）；对四类主要策略——空仓者、持仓者、卖出信号、加仓信号——给出绑定可观察触发条件的专业建议。汇总按 `docs/model/financials—summary-template.json` 落盘到 `research/companies/<company-id>/`。

**完成标准**：七个维度的结论与评分齐全且打分依据可追溯到前两步具体字段；四类策略建议齐全且触发条件可观察、可执行；引用的价格与阈值均来自前两步经工具验算的数字；数据截止时间已声明。
...
执行 `docs/research/workflow/07-evaluation-and-feedback.md`（本步骤唯一正文）。阅读评分**默认由评分 Agent 完成，不逐题问操作者**（2026-09-06 起）：操作者没法冷静评价自己刚亲手产出的报告，评分 Agent 是只读成品页的干净读者。步骤是：用 Task 工具启动**一个评分 Agent**，只读第 5 步渲染的成品页（`research/site/companies/<company-id>.html`）与该公司的上一轮评估基线，按五个维度（可信/洞察/好读/可执行/密度）与三个必填非评分字段（比上一份、最差的一处、缺陷归属步骤）打分，返回单个 JSON；主会话向操作者转述并**当场问行为两问**（是否改变仓位 / 是否熟悉行业——只有操作者本人能如实回答，不答记 null 不阻塞），补模型名后落盘 `tmp/<company-id>-rating.json`，跑 `npm run research:feedback -- --company <company-id> --rating-json tmp/<company-id>-rating.json`（该命令同时重跑发布，让研究评估页立刻包含这次评分）；人工逐题打分只在离线补录时走交互式命令。评估记录按评分源（subagent / operator）分行记账，评估页按行展示评分人——**分数只能按同一评分源的时间序列读趋势，不跨源横比**。
...
**完成标准**：`research/evals/runs.jsonl` 与 `defects.jsonl` 各新增一行；五项评分、相对比较、「最差的一处」均已填写（行为两问未答时记 null）；研究评估页能看到这一行；`npm test` 通过。

全部完成后向用户汇报：七个维度的结论与信心度、四类策略建议、数据截止时间与页面链接，并保留不构成个性化投资建议的声明——内容以第 3 步总结文件为准，不在汇报里新增页面上没有的判断。
