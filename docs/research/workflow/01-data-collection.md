...
- **同一份公司文件内的金额记数法保持一致**：中文量级（`"1044.61亿"`）与英文缩写（`"104.46B"`）两套都对，混在同一份文件里不是错值，但同一页上要读者自己换算量级。按该文件里占多数的那套写，少数派改写过去（只改表现形式，数值语义不变）。第 4 步的写法闸门会按文件报 `[同一文件内记数法混用]`：
  - ❌ 同一份 `hk-2015` 文件里 `marketCap.reported` 写 `"101.22B"`、`marketCap.computed` 写 `"1044.61亿"`——同一个指标、同一页、两套量级
  - ✅ 两处都用英文缩写（`"101.22B"` / `"104.46B"`）；整份文件都用 `亿` 的港股中文报告同样合规——判定基准是**同一文件内是否两套并存**，不是上市地
  - 计数类单位（股 / 辆 / 户 / MAU）不参与比较（中文里「3.49亿股」是自然写法），`source1` / `source2` 里照抄来源原文的数值副本也不参与
- **没有单位就把 `unit` / `currency` 写 `null`，不要用占位符字符串**：`"-"` / `"n/a"` / `"—"` 非空，渲染层拿它当单位拼在数值后面，`currency` 位上的占位符还会压过真正的 `unit`——闸门报 `[单位写成占位符]`：
  - ❌ `"sharesOutstanding": { "value": "43.3亿", "unit": "股", "currency": "-" }`（渲染成 `43.3亿 -`，快手 `hk-1024` 就是这样发出去的）
