...
**写法闸门（全量叶子扫描，独立于分数，脚本自动执行）**：首屏闸门只看四个字段，但被读到的字段远不止四格。脚本扫描三份文件的每个叶子节点，挡五类「页面认得出、读者看到的却是错的」写法，命中即计 0 分权重并记入缺口清单，**不放行**：
