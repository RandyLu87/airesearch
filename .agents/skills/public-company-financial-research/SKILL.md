...
3. 严格按以下规则确定路径，不自行发明名称：
   - 港股：`research/companies/hk-<4至5位代码>-<英文slug>/YYYY-MM-DD-HHMM-analysis.md`
   - 美股：`research/companies/us-<小写ticker>-<英文slug>/YYYY-MM-DD-HHMM-analysis.md`
   - A股：`research/companies/<sh|sz|bj>-<6位代码>-<英文slug>/YYYY-MM-DD-HHMM-analysis.md`
   - `<英文slug>` 只用小写 ASCII 字母、数字和连字符；ticker 去掉标点。时间使用 Asia/Shanghai 的 24 小时制创建时刻。
   - 同一公司同一天只保留一个研究文件；若当天已有文件，把后续更新追加到该文件，不创建 `v2`、`new`、`final` 或其他变体。
...
完成研究文件后运行 `python3 .codex/hooks/validate_research_paths.py`。若校验失败，立即修正路径或文件名，不询问用户是否需要修正。
